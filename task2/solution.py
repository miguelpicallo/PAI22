        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    # Sum losses fore different Monte Carlo samples
                    # Initialize the loss
                    loss = torch.tensor(0.0) 
                    for _ in range(10):
                        # Perform forward pass
                        current_logits, log_prior, log_var_posterior = self.network.forward(batch_x)
                        # Add new calculated loss
                        loss = loss + F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum') + (log_var_posterior - log_prior)/num_batches 

                    # Backpropagate to get the gradients
                    loss.backward()    

                # print(self.network.parameters().grad)
                self.optimizer.step()
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = None
        self.prior = UnivariateGaussian(torch.tensor(0.0), torch.tensor(1.0)*0.5)
        # self.prior = ScaleMixGaussian(torch.tensor(0.0), torch.tensor(1.0)*0.5, torch.tensor(1.0)*1e-4, torch.tensor(1.0)*0.75)
        ...
        #       torch.nn.Parameter(torch.zeros((out_features, in_features))),
        #       torch.nn.Parameter(torch.ones((out_features, in_features))))
        self.weights_var_posterior_mu = torch.nn.Parameter(torch.randn((out_features, in_features))) # random initialization to avoid all zeros
        self.weights_var_posterior_rho = torch.nn.Parameter(torch.ones((out_features, in_features))*-2) # small sigma
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior_mu = torch.nn.Parameter(torch.randn((out_features))) # random initialization to avoid all zeros
            self.bias_var_posterior_rho = torch.nn.Parameter(torch.ones((out_features))*-2) # small sigma
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return F.gaussian_nll_loss(values,self.mu*torch.ones((values.size())),self.sigma**2*torch.ones((values.size())),reduction='sum')*-1
        # n = values.size()
        # return -n*1./2*torch.log(2*torch.tensor(np.pi)*self.sigma**2) +torch.sum(-1./2*(values-self.mu)**2/self.sigma**2)
        # return -0.5*torch.log(torch.prod(self.sigma))

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        # return self.mu
        # return torch.normal(self.mu,self.sigma)
        return self.mu + self.sigma*torch.normal(0,1)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return F.gaussian_nll_loss(values,self.mu,F.softplus(self.rho)**2,reduction='sum')*-1
        # sigma = F.softplus(self.rho)
        # n = values.size()
        # return -n*1./2*torch.log(2*torch.tensor(np.pi)) +torch.sum(-1./2*torch.log(sigma**2) -1./2*(values-self.mu)**2/sigma**2)

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        # return self.mu
        # return torch.normal(self.mu,F.softplus(self.rho))
        return self.mu + F.softplus(self.rho)*torch.normal(0,1,self.mu.size())
        raise NotImplementedError()

class ScaleMixGaussian(ParameterDistribution): # to be tried
    """
    Scale mixture Gaussian distribution.
    For multivariate data, this assumes all elements to be i.i.d.
    """

    def __init__(self, mu: torch.Tensor, sigma1: torch.Tensor, sigma2: torch.Tensor, pi: torch.Tensor):
        super(ScaleMixGaussian, self).__init__()  # always make sure to include the super-class init call!
        assert mu.size() == () and sigma1.size() == () and sigma2.size() == () and pi.size() == ()
        assert sigma1 > 0 and sigma2>0 and pi>0 and pi<1
        self.mu = mu
        self.sigma1 = sigma1
        self.sigma2 = sigma2
        self.pi = pi
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        logprob1 = F.gaussian_nll_loss(values,self.mu*torch.ones((values.size())),self.sigma1**2*torch.ones((values.size())),reduction='sum')*-1 + torch.log(self.pi)
        logprob2 = F.gaussian_nll_loss(values,self.mu*torch.ones((values.size())),self.sigma2**2*torch.ones((values.size())),reduction='sum')*-1 + torch.log(1-self.pi)
        if logprob1>logprob2:
            return logprob1 + torch.log(1+torch.exp(logprob2-logprob1))
        else:
            return logprob2 + torch.log(1+torch.exp(logprob1-logprob2))
        # return torch.logsumexp(torch.cat((logprob1,logprob2),0))
        # n = values.size()
        # return -n*1./2*torch.log(2*torch.tensor(np.pi)*self.sigma**2) +torch.sum(-1./2*(values-self.mu)**2/self.sigma**2)
        # return -0.5*torch.log(torch.prod(self.sigma))
    ...
    def sample(self) -> torch.Tensor:
        ...
        # return self.mu
        # return torch.normal(self.mu,self.sigma)
        if np.random.rand()<self.pi.numpy()[0]:
            return self.mu + self.sigma1*torch.normal(torch.tensor(0.0),torch.tensor(1.0))
        else:
            return self.mu + self.sigma2*torch.normal(torch.tensor(0.0),torch.tensor(1.0))
        raise NotImplementedError()
