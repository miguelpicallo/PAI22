...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        # return 0,0,0
        with torch.no_grad():
            act_distrib = self.pi._distribution(state)
            act = act_distrib.sample()
            log_prob = self.pi._log_prob_from_distribution(act_distrib,act)
            return act.detach().numpy(),self.v.forward(state).detach().numpy(),log_prob.detach().numpy()
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # the discount_cumsum function at the top of this file. 
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        for i in range(ret.size(dim=0)):
            # loss -= ret[i]*self.ac.pi.forward(obs[i],act[i])[1] # case2: reward-to-go
            loss -= phi[i]*self.ac.pi.forward(obs[i],act[i])[1] # case8: use the baseline
        loss.backward()
    ...
    def v_update(self, data):
        ...
        for rounds in range(100):
            loss = torch.tensor(0.0)
            for i in range(ret.size(dim=0)):
                loss = 1./2*(ret[i] - self.ac.v.forward(obs[i]))**2 
            loss.backward()
    ...
    def get_action(self, obs):
        ...
        act_distrib = self.ac.pi._distribution(torch.as_tensor(obs, dtype=torch.float32))
        return act_distrib.sample().detach().numpy()
