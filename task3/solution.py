...
class BO_algo(object):
    def __init__(self):
        ...
        self.constraint_model = GaussianProcessRegressor(kernel=ConstantKernel(3.5,constant_value_bounds="fixed") 
                                                          * RBF(2.0,length_scale_bounds="fixed"),
                                                           alpha = 5e-3) # TODO : GP model for the constraint function
        self.objective_model =  GaussianProcessRegressor(kernel=ConstantKernel(1.5,constant_value_bounds="fixed") 
                                                          * RBF(1.5,length_scale_bounds="fixed"),
                                                           alpha = 1e-2) # TODO : GP model for your acquisition function
        # -> objective instead of adquisition?
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # UCB + penalization based on violation probability
        # return (-f_mean + 1*f_std)[0] - 1e10*(c_mean+c_std*2>0)[0] # UCB penalized with large value when probability of violation is not negligible -> passed
        # return (-f_mean + 1*f_std)[0] - 1e10*np.maximum((c_mean+c_std*2)[0],0)**2 # UCB penalized with conditunous differentiable function
        # return (-f_mean + 1*f_std)[0] - 1e10*(1-norm.cdf((-c_mean/c_std)[0])) # UCB with probability of feasible
        
        # Expected improvement * probability of feasibility 
        prob_feas = norm.cdf((-c_mean/c_std)[0])
        f_best = -1e5
        points = np.array(self.previous_points)
        if len(points) != 0:
            f_best = np.min(points[:,2])
            idx_feas = np.where(points[:,3]<0)[0] # index of feasible points gathered so far
            if len(idx_feas) < len(points)*0.10: # if only a few feasible points
                return prob_feas # return only probability to find the feasible space -> passed with no violations
            if len(idx_feas) < len(points)*0.15: # also look outside the known feasible set
                return prob_feas*np.sum([np.linalg.norm(x-points[i,:2],2) for i in idx_feas]) # -> better regret
        z = ((f_best-f_mean)/f_std)[0]
        ei = (z*norm.cdf(z) + norm.pdf(z))*f_std # expected improvement
        pi = norm.cdf(z) # probability of improvement, not used
        return ei*prob_feas
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        self.constraint_model.fit(np.array(self.previous_points)[:,:2],np.array(self.previous_points)[:,3])
        self.objective_model.fit(np.array(self.previous_points)[:,:2],np.array(self.previous_points)[:,2])
        # raise NotImplementedError
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # TODO: enter your code here
        points = np.array(self.previous_points)
        idx_feas = np.where(points[:,3]<0)[0] # include measurement noise?
        if len(idx_feas) == 0: # return 0,0 if no feasible observed
            return np.array([0,0])
        points_feas = points[idx_feas,:] 
        return points_feas[np.argmin(points_feas[:,2]),:2]
        raise NotImplementedError
